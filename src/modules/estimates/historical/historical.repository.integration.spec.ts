...
describeIfDb('HistoricalRepository (PostgreSQL)', () => {
  ...
  beforeAll(async () => {
    dataSource = new DataSource({
      ...
      migrations: [InitialSchema1740000000000, EstimatesEngine1750000000000, HistoricalCollection1785279091429],
    });
    await dataSource.initialize();
    await dataSource.runMigrations();
    [{ id: bridgeId }] = await dataSource.query<Array<{ id: string }>>(
      `INSERT INTO "bridges" ("name", "slug", "status", "sortOrder") VALUES ('HBTR fixture', 'hbtr-final-fixture', 'low', 99) RETURNING "id"`,
    );
  });
  ...
  it('uses DST-correct local bins, per-date medians, excludes NULL slots, and reports closures separately', async () => {
    const insert = async (
      ...
      isOpen = true,
      slotStart: string | null = fetchedAt,
    ): Promise<void> => {
    ...
    const [pattern] = await new HistoricalRepository(dataSource).findPatterns(
      LaneType.General,
      1,
      '06:00',
    ...
    expect(pattern).toEqual(
      expect.objectContaining({
        ...
        p75: 45,
        observationCount: 4,
        comparableLocalDates: 4,
        coverageStart: '2026-03-02',
        coverageEnd: '2026-03-23',
        coverageRatio: 1,
      }),
    );
  ...
  it('uses the collector partial index for the slot-owned historical scan', async () => {
    ...
    const plan = await dataSource.query<Array<{ 'QUERY PLAN': string }>>(
      `EXPLAIN SELECT "bridgeId" FROM "cbp_snapshots" WHERE "slotStart" IS NOT NULL AND "laneType" = $1`,
      [LaneType.General],
    );
