...
export interface CbpHistoryAuditResult {
  ...
  uniqueFetchInstants: number; // distinct (bridgeId, laneType, localDate, localHour) combos
...
export function computeCbpHistoryAuditMetrics(
  ...
  const uniqueInstantKeys = new Set<string>();
  ...
  for (const r of rows) {
    ...
    uniqueInstantKeys.add(`${r.bridgeId}|${r.laneType}|${localDate}|${localHour}`);
  ...
  return {
    ...
    minFetchedAt,
    maxFetchedAt,
    uniqueFetchInstants: uniqueInstantKeys.size,
